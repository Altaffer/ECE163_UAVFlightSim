...
if __name__=="__main__":
	...
	assert( deep_compare([[3, 4],3,1], [[3,4] ,3,1]) )
	assert( not deep_compare( 4, 5) )
	assert( not deep_compare([3,3,1], [3,5,1]) )
	assert( not deep_compare([[3, 4],3,1], [[3,5] ,3,1]) )
...
# 				ttprint(DETAIL, f"  Failed test {red}{name}{white}")
	...
	def compare_outputs(self, observed, expected, rel_tol = 1e-9, abs_tol=1e-9):
		...
		if set(observed.keys()) != set(expected.keys()):
			raise Exception(f"observed and expected outputs have different keys!  observed={observed.keys()}, expected={expected.keys()}")
